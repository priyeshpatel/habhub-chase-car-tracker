...
public class TrackerService extends Service
{
		...
		public void onLocationChanged(Location l)
		{
			...
			new Thread(new Runnable()
			{
				public void run()
				{
					...
					ListenerTelemetry telemetry = new ListenerTelemetry(callsign, location, time, getDevice(),
							getClient());
		...
		for (int i = messengerClients.size() - 1; i >= 0; i--)
		{
			try
			{
				...
				b.putString("latitude",
						Double.toString(Double.valueOf(new DecimalFormat("#.######").format(l.getLatitude()))));
				b.putString("longitude",
						Double.toString(Double.valueOf(new DecimalFormat("#.######").format(l.getLongitude()))));
				b.putString("altitude", Long.toString(Math.round(l.getAltitude())));
				b.putString("speed",
						Float.toString(Float.valueOf(new DecimalFormat("#.##").format((l.getSpeed() / 1000) * 3600))));
	...
	private void showNotification()
	{
		...
		PendingIntent pi = PendingIntent.getActivity(this, 0, new Intent(this, Main.class),
				Notification.FLAG_ONGOING_EVENT);
		n.flags = Notification.FLAG_ONGOING_EVENT;
		n.setLatestEventInfo(this, getText(R.string.service_notification_title),
				getText(R.string.service_notification), pi);
